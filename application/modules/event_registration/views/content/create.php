    ...
    <div class="col-xs-12 col-sm-6 col-md-6">
        <div class="form-group">
             <?php if (is_array($categories_select) && count($categories_select)) :
                echo form_dropdown(array('class'=>'form-control', 'id'=>'category', 'name'=>'category'), $categories_select, set_value('category', isset($event_registration->category) ? $event_registration['category'] : ''), 'Category'. lang('bf_form_label_required'), "tabindex='1'");
            endif; ?>
        </div>    
        <div class="form-group">
            <?php 
                $options = array('-1'=>'Select one');
                echo form_dropdown(array('class'=>'form-control', 'id'=>'style', 'name'=>'style'), $options, set_value('style', isset($event_registration->style) ? $event_registration->style : ''), 'Style'. lang('bf_form_label_required'), "tabindex='2'");
            ?>
        </div>    
        <div class="form-group">
            <?php 
                $options = array('-1'=>'Select one');
                echo form_dropdown(array('class'=>'form-control', 'id'=>'performance', 'name'=>'performance'), $options, set_value('category', isset($event_registration->performance) ? $event_registration->performance : ''), 'Performance'. lang('bf_form_label_required'), "tabindex='3'");
            ?>
        </div>
        <h4>Assign Participants</h4>
        <div class="form-group">
            <?php  if (is_array($teams_select) && count($teams_select)) :
                    echo form_dropdown(array('class'=>'form-control', 'id'=>'team[]', 'name'=>'team[]', 'multiple'=>'multiple'), $teams_select, set_value('team', isset($registration_team->team) ? $registration_team->team : ''), lang('event_registration_field_team') . lang('bf_form_label_required'), "tabindex='1'");
            endif; ?>
        </div>
        ...
        <div class="form-group<?php echo form_error('attach_1') ? ' error' : ''; ?>">
            ...
            <div class='controls'>
                <input class="btn btn-primary" id='attach_1' type='file' name='attach[]' maxlength='4000' value="<?php echo set_value('attach_1', isset($event_registration->attach_1) ? $event_registration->attach_1 : ''); ?>" />
