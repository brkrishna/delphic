...
{
    protected $permissionCreate = 'Todo.Content.Create';
    protected $permissionDelete = 'Todo.Content.Delete';
    protected $permissionEdit   = 'Todo.Content.Edit';
    protected $permissionView   = 'Todo.Content.View';
    ...
    public function index($offset = 0)
    {
        ...
        if (isset($_POST['delete'])) {
            ...
            if (is_array($checked) && count($checked)) {
                ...
                foreach ($checked as $pid) {
                    $deleted = $this->todo_model->delete($pid);
                    if ($deleted == false) {
                        $result = false;
                    }
                }
        ...
        $pagerUriSegment = 5;
        $pagerBaseUrl = site_url(SITE_AREA . '/content/todo/index') . '/';
        
        $limit  = $this->settings_lib->item('site.list_limit') ?: 15;

        $this->load->library('pagination');
        $pager['base_url']    = $pagerBaseUrl;
        $pager['total_rows']  = $this->todo_model->count_all();
        $pager['per_page']    = $limit;
        $pager['uri_segment'] = $pagerUriSegment;

        $this->pagination->initialize($pager);
        $this->todo_model->limit($limit, $offset);        
        
        $records = $this->todo_model->find_all();
        
        Template::set('records', $records);
        
        Template::set('toolbar_title', lang('todo_manage'));
    ...
    public function create()
    {
        ...
        if (isset($_POST['save'])) {
            if ($insert_id = $this->save_todo()) {
                log_activity($this->auth->user_id(), lang('todo_act_create_record') . ': ' . $insert_id . ' : ' . $this->input->ip_address(), 'todo');
            ...
            if ( ! empty($this->todo_model->error)) {
                Template::set_message(lang('todo_create_failure') . $this->todo_model->error, 'error');
            }
        }

        Template::set('toolbar_title', lang('todo_action_create'));
    ...
    public function edit()
    {
        ...
        if (isset($_POST['save'])) {
            ...
            if ($this->save_todo('update', $id)) {
                log_activity($this->auth->user_id(), lang('todo_act_edit_record') . ': ' . $id . ' : ' . $this->input->ip_address(), 'todo');
        ...
        elseif (isset($_POST['delete'])) {
            ...
            if ($this->todo_model->delete($id)) {
                log_activity($this->auth->user_id(), lang('todo_act_delete_record') . ': ' . $id . ' : ' . $this->input->ip_address(), 'todo');
    ...
    private function save_todo($type = 'insert', $id = 0)
    {
        ...
        // Validate the data
        $this->form_validation->set_rules($this->todo_model->get_validation_rules());
        ...
        $data = $this->todo_model->prep_data($this->input->post());
