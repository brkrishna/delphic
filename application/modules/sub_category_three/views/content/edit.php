...
<div class='admin-box'>
    ...
    <?php echo form_open($this->uri->uri_string(), 'class="form-horizontal"'); ?>
        ...
        <fieldset class='form-actions'>
            <input type='submit' name='save' class='btn btn-primary' value="<?php echo lang('sub_category_three_action_edit'); ?>" />
            <?php echo lang('bf_or'); ?>
            <?php echo anchor(SITE_AREA . '/content/sub_category_three', lang('sub_category_three_cancel'), 'class="btn btn-warning"'); ?>
            
            <?php if ($this->auth->has_permission('Sub_Category_Three.Content.Delete')) : ?>
                <?php echo lang('bf_or'); ?>
                <button type='submit' name='delete' formnovalidate class='btn btn-danger' id='delete-me' onclick="return confirm('<?php e(js_escape(lang('sub_category_three_delete_confirm'))); ?>');">
                    <span class='icon-trash icon-white'></span>&nbsp;<?php echo lang('sub_category_three_delete_record'); ?>
                </button>
            <?php endif; ?>
        </fieldset>
    <?php echo form_close(); ?>
